...
from util import channelBound, chargesToColor, rotateAndGetOffset
...
filterFeeds = {"white": {"white": (1, 0, 0), "blue": (0, 1, 0), "red": (0, 0, 1)}, "blue": {"white": (0, 0.5, 0), "blue": (0, 1, 0), "red": (0, 0, 0)}, "red": {"white": (0, 0, 0.5), "blue": (0, 0, 0), "red": (0, 0, 1)}}
...
def init():
    ...
    for i in range(1, IMPACT_FRAMES + 1):
        laserImpactIMGsRaw.append(get_asset(f"LaserImpact{i}"))
...
class Player:
    def __init__(self, defaultZooms, x, y, dimensions=[10, 30]):
        ...
        for zoom in self.defaultZooms:
            ...
            self._impactIMGs[zoom] = [pygame.transform.scale(img, (size, size)) for img in laserImpactIMGsRaw]
    ...
    def tick(self, frameLength, cTerrain: terrain.Terrain, mousePos, keysDown, events):
        ...
        for lase in self.laser:
            locked = lase.updateLaser(
                cTerrain,
                self.x - SPRITE_WIDTH / 2 + ARM_PIVOT_X + self.laserAttributes.distance * math.cos(self.armAngle),
                self.y - SPRITE_HEIGHT / 2 + ARM_PIVOT_Y + self.laserAttributes.distance * math.sin(-self.armAngle),
                -self.armAngle,
            )
            ...
            if lase.damageFrame:
                ...
                if lase.collision:
                    ...
                    cTerrain.newPlayerDamageCircles.append(
                        [laserProperties.getLaserDMG(self.laserAttributes, self.laserFirstHit, self.laserRamps), x, y, self.laserAttributes.DMGRange, self.laserAttributes.areaDMGFalloff]
                    )
