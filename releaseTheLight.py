...
class Game:
    def __init__(self, window: pygame.Surface, fps=60, full_world=True, dev_mode=False, loading_screen: loading_screen.LoadingScreen = None):
        ...
        if not full_world:
            ...
            self.WORLD_HEIGHT = 10 * CHUNK_SIZE
        # high temporarily
    ...
    def update_cam_pos(self, fps, zoom, player):
        ...
        player_x = player.x
        player_y = player.y
        old_focus = self.cam_focus

        focus_half_w = half_win_w * 0.3
        focus_half_h = half_win_h * 0.3
        if player.laser:
        ...
        if self.cam_focus == "laser":
            ...
            focus_y = 0
            if player.x_speed > 0.18:
                self.cam_focus = "right"
            elif player.x_speed < -0.18:
                self.cam_focus = "left"
            #else:
            ...
            if self.cam_focus == "right":
                focus_x = - half_win_w * 0.3
            elif self.cam_focus == "left":
                focus_x = half_win_w * 0.3
            else:
                focus_x = 0
                focus_half_w = half_win_w * 0.5
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if self.developing_mode:
                        if event.key == pygame.K_z:
                            ...
                            self.game_world.player.add_charge(25, added_dist)
